...
<div class="max-w-6xl mx-auto">
    ...
    <form method="GET" action="{{ route('reports.sales') }}" class="bg-white p-4 rounded shadow mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex gap-2 justify-end">
                {{-- Fix route names --}}
                <a href="{{ route('reports.sales.excel', request()->all()) }}" class="bg-green-600 text-white px-3 py-2 rounded">Export Excel</a>
                <a href="{{ route('reports.sales.pdf', request()->all()) }}" class="bg-gray-700 text-white px-3 py-2 rounded">Export PDF</a>
            </div>
        </div>
    </form>
    ...
    <div class="mb-4">
        ...
        {{-- Fix variable names --}}
        <p>Total Sales: GHS {{ number_format($total,2) }} | Total Paid: GHS {{ number_format($paid,2) }} | Outstanding: GHS {{ number_format($outstanding,2) }}</p>
    </div>
        ...
        <table class="min-w-full">
            ...
            @forelse($sales as $sale)
                <tr class="border-t">
                    ...
                    <td class="p-3">{{ $sale->created_at->toDayDateTimeString() }}</td>
                </tr>
            @empty
                <tr>
